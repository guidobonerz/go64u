...
func DownloadCommand() *cobra.Command {
	...
	cmd.Flags().StringVarP(&filter.Year, "year", "", "", "defaults to current year")
	cmd.Flags().StringVarP(&filter.Rating, "rating", "", "", "")
	cmd.Flags().StringVarP(&filter.Type, "type", "", "", "")
	cmd.Flags().StringVarP(&filter.Latest, "latest", "", "", "default to current month")
	cmd.Flags().IntVarP(&offset, "offset", "", 0, "")
	cmd.Flags().IntVarP(&limit, "limit", "", 80, "")
